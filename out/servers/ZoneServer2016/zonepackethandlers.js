...
exports.zonePacketHandlers = void 0;
const debug = require("debug")("ZoneServer");
const utils_1 = require("../../utils/utils");
const craftmanager_1 = require("./managers/craftmanager");
const enums_1 = require("./models/enums");
const basefullcharacter_1 = require("./entities/basefullcharacter");
...
class zonePacketHandlers {
    ...
    ClientFinishedLoading(server, client, packet) {
        ...
        if (client.firstLoading) {
            client.character.lastLoginDate = (0, utils_1.toHex)(Date.now());
            server.setGodMode(client, false);
            setTimeout(() => {
                server.sendAlert(client, "Welcome to H1emu! :D");
                server.sendChatText(client, `server population : ${utils_1._.size(server._characters)}`);
                if (client.isAdmin) {
            ...
            server.sendData(client, "Synchronization", {
                serverTime: (0, utils_1.Int64String)(server.getServerTime()),
                serverTime2: (0, utils_1.Int64String)(server.getServerTime()),
            });
    ...
    CommandRecipeStart(server, client, packet) {
        new craftmanager_1.CraftManager(client, server, packet.data.recipeId, packet.data.count);
    }
    CommandSpawnVehicle(server, client, packet) {
    ...
    CollisionDamage(server, client, packet) {
        const characterId = packet.data.characterId, damage = packet.data.damage, vehicle = server._vehicles[characterId];
    ...
    CommandPointAndReport(server, client, packet) {
        debug(packet);
    ...
    CommandReportLastDeath(server, client, packet) {
        const targetClient = client.lastDeathReport?.attacker;
        if (!client.lastDeathReport)
            return;
        if (!targetClient) {
            server.sendChatText(client, "Client not found.");
            return;
        ...
        targetClient.reports += 1;
        const logs = [];
        targetClient.clientLogs.forEach((log) => {
            if (log.isSuspicious) {
                logs.push(log.log);
            }
        });
        const obj = [
            ...
            },
            {
                ...
                info: `Shots fired:${targetClient.pvpStats.shotsFired}, shots hit:${targetClient.pvpStats.shotsHit}, overall accuracy: ${((100 * targetClient.pvpStats.shotsHit) /
                    targetClient.pvpStats.shotsFired).toFixed(2)}% | head: ${((targetClient.pvpStats.head * 100) /
                    targetClient.pvpStats.shotsHit).toFixed(0)}% | spine: ${((targetClient.pvpStats.spine * 100) /
                    targetClient.pvpStats.shotsHit).toFixed(0)}% | hands: ${((targetClient.pvpStats.hands * 100) /
                    targetClient.pvpStats.shotsHit).toFixed(0)}% | legs ${((targetClient.pvpStats.legs * 100) /
                    targetClient.pvpStats.shotsHit).toFixed(0)}%`,
            },
            { title: "Reported player suspicious processes:", info: `:${logs}` },
            {
                title: "Reported by:",
    ...
    KeepAlive(server, client, packet) {
        if (client.isLoading && client.characterReleased) {
            setTimeout(() => {
                ...
                if (client.routineInterval || !client.characterReleased)
                    return;
    ...
    ClientUpdateMonitorTimeDrift(server, client, packet) {
        // nothing for now
    }
    ClientLog(server, client, packet) {
        if (packet.data.file === "Synchronization.log") {
            if (packet.data.message
                ...
                const interval = Number(pruned.replace(pruned.match(/\d+/).join(""), "").replace(" s", ""));
                if (!server._soloMode) {
                    (0, utils_1.logClientActionToMongo)(server._db?.collection(enums_2.DB_COLLECTIONS.FAIRPLAY), client, server._worldId, {
                        type: "time drifted",
                        drifted,
                        interval,
                ...
                server.sendChatTextToAdmins(`FairPlay: ${client.character.name} time drifted forward ${drifted} s span of ${interval} s, accelerating by: ${((drifted / interval) *
                    100).toFixed(0)}%`, false);
            }
        }
        if (packet.data.file === "ClientProc.log" &&
            ...
            for (let x = 0; x < suspicious.length; x++) {
                if (packet.data.message.toLowerCase().includes(suspicious[x])) {
                    ...
                    if (!server._soloMode) {
                        (0, utils_1.logClientActionToMongo)(server._db?.collection(enums_2.DB_COLLECTIONS.FAIRPLAY), client, server._worldId, { type: "suspicious software", suspicious: suspicious[x] });
                    }
                    server.sendChatTextToAdmins(`FairPlay: ${client.character.name} is using suspicious software - ${suspicious[x]}`, false);
                }
            }
    ...
    Synchronization(server, client, packet) {
        const serverTime = (0, utils_1.Int64String)(server.getServerTime());
        const reflectedPacket = {
            ...packet.data,
            serverTime: serverTime,
            serverTime2: serverTime,
            time3: (0, utils_1.Int64String)(Number(packet.data.clientTime)) + 2,
        };
    ...
    CommandInteractRequest(server, client, packet) {
        ...
        if (!entity)
            return;
        const isConstruction = entity instanceof constructionparententity_1.ConstructionParentEntity ||
            entity instanceof constructiondoor_1.ConstructionDoor;
        if (!(0, utils_1.isPosInRadius)(isConstruction ? 4 : server._interactionDistance, client.character.state.position, isConstruction
    ...
    async WallOfDataClientSystemInfo(server, client, packet) {
    ...
    PlayerUpdateManagedPosition(server, client, packet) {
        const characterId = server._transientIds[packet.data.transientId], vehicle = characterId ? server._vehicles[characterId] : undefined;
        if (!vehicle)
            return;
        //if (!server._soloMode) {
        server.sendDataToAllOthersWithSpawnedEntity(server._vehicles, client, characterId, "PlayerUpdatePosition", {
    ...
    VehicleStateData(server, client, packet) {
        server.sendDataToAllOthersWithSpawnedEntity(server._vehicles, client, packet.data.guid, "Vehicle.StateData", {
            ...packet.data,
        });
    }
    PlayerUpdateUpdatePositionClientToZone(server, client, packet) {
        ...
        if (packet.data.stance) {
            if (packet.data.stance == enums_1.Stances.STANCE_XS) {
                const pos = client.character.state.position;
                if (!server._soloMode) {
                    (0, utils_1.logClientActionToMongo)(server._db?.collection(enums_2.DB_COLLECTIONS.FAIRPLAY), client, server._worldId, { type: "XS glitching", pos });
                }
                server.sendChatTextToAdmins(`FairPlay: Possible XS glitching detected by ${client.character.name} at position [${pos[0]} ${pos[1]} ${pos[2]}]`);
                setTimeout(() => {
            ...
            client.character.isRunning =
                packet.data.stance == enums_1.Stances.MOVE_STANDING_SPRINTING ? true : false;
            const penaltiedStances = [
                enums_1.Stances.JUMPING_BACKWARDS,
                enums_1.Stances.JUMPING_BACKWARDS_LEFT,
                enums_1.Stances.JUMPING_BACKWARDS_RIGHT,
                enums_1.Stances.JUMPING_FORWARD_LEFT,
                enums_1.Stances.JUMPING_FORWARD_RIGHT,
                enums_1.Stances.JUMPING_FORWARD_SPRINTING,
                enums_1.Stances.JUMPING_LEFT,
                enums_1.Stances.JUMPING_RIGHT,
                enums_1.Stances.JUMPING_STANDING,
                enums_1.Stances.JUMPING_WORWARD,
                enums_1.Stances.JUMPING_FORWARD_LEFT_SPRINTING,
                enums_1.Stances.JUMPING_FORWARD_RIGHT_SPRINTING,
            ];
            if (penaltiedStances.includes(packet.data.stance)) {
                client.character._resources[enums_1.ResourceIds.STAMINA] -= 12; // 2% stamina jump penalty
                if (client.character._resources[enums_1.ResourceIds.STAMINA] < 0)
        ...
        if (movingCharacter) {
            server.sendRawToAllOthersWithSpawnedCharacter(client, movingCharacter.characterId, server._protocol.createPositionBroadcast2016(packet.data.raw, movingCharacter.transientId));
        }
        if (packet.data.position) {
            ...
            if (client.hudTimer != null &&
                !(0, utils_1.isPosInRadius)(1, client.character.state.position, client.posAtLogoutStart)) {
            ...
            if (client.character.mountedContainer &&
                ...
                if (!(0, utils_1.isPosInRadius)(client.character.mountedContainer.interactionDistance, client.character.state.position, client.character.mountedContainer.state.position)) {
                    client.character.dismountContainer(server);
                }
            }
    ...
    CharacterRespawn(server, client, packet) {
        this.commandHandler.executeInternalCommand(server, client, "respawn", packet);
    }
    CharacterFullCharacterDataRequest(server, client, packet) {
        const entity = server.getEntity(packet.data.characterId);
        if (!(entity instanceof basefullcharacter_1.BaseFullCharacter) && !(entity instanceof plant_1.Plant)) {
            return;
        }
    ...
    LockssetLock(server, client, packet) {
        if (!client.character.currentInteractionGuid || packet.data.password === 1)
            return;
        const doorEntity = server._constructionDoors[client.character.currentInteractionGuid];
        if (!doorEntity)
            return;
    ...
    VehicleCurrentMoveMode(server, client, packet) {
        const { characterId, moveMode } = packet.data, vehicle = server._vehicles[characterId];
        if (!vehicle)
            return;
        debug(`vehTransient:${vehicle.transientId} , mode: ${moveMode} from ${client.character.name} time:${Date.now()}`);
    }
    ...
    ConstructionPlacementFinalizeRequest(server, client, packet) {
        ...
        const matrix = (0, utils_1.quat2matrix)(array);
        const euler = [
            Math.atan2(matrix[7], matrix[8]),
            Math.atan2(-matrix[6], Math.sqrt(Math.pow(matrix[7], 2) + Math.pow(matrix[8], 2))),
        ...
        const modelId = server.getItemDefinition(packet.data.itemDefinitionId).PLACEMENT_MODEL_ID;
    ...
    CommandItemDefinitionRequest(server, client, packet) {
    ...
    CharacterWeaponStance(server, client, packet) {
        if (client.character.positionUpdate) {
            client.character.positionUpdate.stance = packet.data.stance;
        }
        server.sendDataToAllOthersWithSpawnedEntity(server._characters, client, client.character.characterId, "Character.WeaponStance", {
            characterId: client.character.characterId,
            stance: packet.data.stance,
        });
    }
    FirstTimeEventInventoryAccess(server, client, packet) {
    ...
    CommandSuicide(server, client, packet) {
    ...
    //#region ITEMS
    RequestUseItem(server, client, packet) {
        debug(packet.data);
        if (packet.data.itemSubData?.count < 1)
            return;
        ...
        if (loadoutSlotId &&
            ...
            utils_1._.size(client.character._containers[loadoutSlotId].items) != 0) {
        ...
        switch (packet.data.itemUseOption) {
            case enums_1.ItemUseOptions.DROP:
            case enums_1.ItemUseOptions.DROP_BATTERY:
            case enums_1.ItemUseOptions.DROP_SPARKS:
                server.dropItem(client, item, packet.data.itemSubData?.count);
                break;
            case enums_1.ItemUseOptions.SLICE:
                server.sliceItem(client, item);
                break;
            case enums_1.ItemUseOptions.EQUIP:
                const activeSlotId = client.character.getActiveLoadoutSlot(itemGuid);
                let loadoutSlotId = client.character.getAvailableLoadoutSlot(server, item.itemDefinitionId);
                if (server.isWeapon(item.itemDefinitionId)) {
                    if (container) {
                        ...
                        if (!activeSlotId) {
                            server.containerError(client, enums_1.ContainerErrors.UNKNOWN_CONTAINER);
                            return;
                        }
                ...
                break;
            case enums_1.ItemUseOptions.SHRED:
                server.shredItem(client, item);
                break;
            case enums_1.ItemUseOptions.DRINK:
            case enums_1.ItemUseOptions.EAT:
            case enums_1.ItemUseOptions.USE_MEDICAL:
                server.useConsumable(client, item);
                break;
            case enums_1.ItemUseOptions.USE:
                server.useItem(client, item);
                break;
            case enums_1.ItemUseOptions.REFUEL:
                server.refuelVehicle(client, item, packet.data.characterId2);
                break;
            case enums_1.ItemUseOptions.IGNITE:
                server.igniteOption(client, item);
                break;
            case enums_1.ItemUseOptions.UNLOAD:
                if (item.weapon) {
                ...
                break;
            case enums_1.ItemUseOptions.SALVAGE:
    ...
    ContainerMoveItem(server, client, packet) {
        const { containerGuid, characterId, itemGuid, targetCharacterId, count, newSlotId, } = packet.data;
        if (characterId == client.character.characterId) {
            // from client container
            if (characterId == targetCharacterId) {
                // from / to client or mounted container
                const sourceContainer = client.character.getItemContainer(itemGuid), targetContainer = client.character.getContainerFromGuid(containerGuid);
                if (sourceContainer) {
                    // from container
                    ...
                    if (targetContainer) {
                        // to container
                        if (!targetContainer.getHasSpace(server, loadoutItem.itemDefinitionId, count)) {
                            server.containerError(client, enums_1.ContainerErrors.NO_SPACE);
                            return;
                        }
                        ...
                        server.addContainerItem(client.character, loadoutItem, targetContainer, count, false);
                    }
                    else if (containerGuid == "0xffffffffffffffff") {
                        // to loadout
                        const loadoutItem = client.character.getLoadoutItem(itemGuid);
    ...
    NpcFoundationPermissionsManagerEditPermission(server, client, packet) {
        const foundation = server._constructionFoundations[packet.data.objectCharacterId];
        ...
        const obj = foundation.permissions[characterId];
        ...
        Object.values(server._constructionFoundations[packet.data.objectCharacterId]
            .occupiedExpansionSlots).forEach((expansion) => {
        ...
        server.sendData(client, "NpcFoundationPermissionsManagerBase.showPermissions", {
            ...
            permissions: Object.values(foundation.permissions).filter((perm) => perm.characterId != foundation.ownerCharacterId),
        });
    }
    NpcFoundationPermissionsManagerAddPermission(server, client, packet) {
        const foundation = server._constructionFoundations[packet.data.objectCharacterId];
        if (foundation.ownerCharacterId != client.character.characterId &&
        ...
        if (!characterId)
            return;
        let obj = foundation.permissions[characterId];
        if (!obj) {
            obj = {
    ...
    Weapon(server, client, packet) {
        ...
        switch (packet.data.weaponPacket.packetName) {
            case "Weapon.MultiWeapon":
                packet.data.weaponPacket.packet.packets.forEach((p) => {
        ...
        function handleWeaponPacket(p) {
            ...
            switch (p.packetName) {
                case "Weapon.FireStateUpdate":
                    ...
                    // wrench workaround
                    if (weaponItem.itemDefinitionId == enums_1.Items.WEAPON_WRENCH &&
                        client.character.currentInteractionGuid) {
                        if (!client.character.temporaryScrapTimeout &&
                            server.getEntityType(client.character.currentInteractionGuid) ==
                                enums_1.EntityTypes.VEHICLE) {
                            const vehicle = server.getEntity(client.character.currentInteractionGuid);
                            ...
                            if (vehicle &&
                                vehicle._resources[enums_1.ResourceIds.CONDITION] < 100000) {
                    ...
                    // crowbar workaround
                    if (weaponItem.itemDefinitionId == enums_1.Items.WEAPON_CROWBAR &&
                        client.character.currentInteractionGuid) {
                        const entity = server.getLootableEntity(client.character.currentInteractionGuid);
                        if (entity) {
                            ...
                            if (allowedSpawners.includes(entity.lootSpawner) &&
                                !client.character.temporaryScrapTimeout) {
                                const chance = Math.floor(Math.random() * 100) + 1;
                                if (chance <= 70) {
                                    client.character.lootItem(server, server.generateItem(enums_1.Items.METAL_SCRAP));
                    ...
                    if (weaponItem.itemDefinitionId == enums_1.Items.WEAPON_HAMMER_DEMOLITION &&
                        client.character.currentInteractionGuid) {
                        const entity = server.getConstructionEntity(client.character.currentInteractionGuid), permission = entity?.getHasPermission(server, client.character.characterId, enums_1.ConstructionPermissionIds.DEMOLISH);
                        if (entity && !(entity instanceof constructionparententity_1.ConstructionParentEntity)) {
                            if (permission) {
                                entity.destroy(server);
                                client.character.lootItem(server, server.generateItem(entity.itemDefinitionId));
                            }
                            else {
                                server.placementError(client, enums_1.ConstructionErrors.DEMOLISH_PERMISSION);
                            }
                        }
                    }
                    // hammer workaround
                    if (weaponItem.itemDefinitionId == enums_1.Items.WEAPON_HAMMER &&
                        client.character.currentInteractionGuid) {
                        ...
                        if (!client.character.temporaryScrapSoundTimeout) {
                            ...
                            server.updateResourceToAllWithSpawnedEntity(entity.characterId, entity.health, enums_1.ResourceIds.CONSTRUCTION_CONDITION, enums_1.ResourceTypes.CONDITION, server.getConstructionDictionary(entity.characterId));
                            client.character.temporaryScrapSoundTimeout = setTimeout(() => {
                                delete client.character.temporaryScrapSoundTimeout;
                            }, 1000);
                        }
                    ...
                    if (p.packet.firestate == 64) {
                        // empty firestate
                        server.sendRemoteWeaponUpdateDataToAllOthers(client, client.character.transientId, weaponItem.itemGuid, "Update.Empty", {});
                        server.sendRemoteWeaponUpdateDataToAllOthers(client, client.character.transientId, weaponItem.itemGuid, "Update.FireState", {
                            state: {
                                firestate: 64,
                    ...
                    if (!weaponItem.weapon?.ammoCount &&
                        weaponItem.itemDefinitionId != enums_1.Items.WEAPON_BOW_MAKESHIFT &&
                        weaponItem.itemDefinitionId != enums_1.Items.WEAPON_BOW_RECURVE &&
                        weaponItem.itemDefinitionId != enums_1.Items.WEAPON_BOW_WOOD)
                        return;
                    if (p.packet.firestate > 0) {
                        server.sendRemoteWeaponUpdateDataToAllOthers(client, client.character.transientId, weaponItem.itemGuid, "Update.Chamber", {});
                    }
                    server.sendRemoteWeaponUpdateDataToAllOthers(client, client.character.transientId, weaponItem.itemGuid, "Update.FireState", {
                        state: {
                ...
                case "Weapon.Fire":
                    ...
                    server.stopHudTimer(client);
                    server.sendRemoteWeaponUpdateDataToAllOthers(client, client.character.transientId, weaponItem.itemGuid, "Update.ProjectileLaunch", {});
                    const projectilesCount = server.getWeaponAmmoId(weaponItem.itemDefinitionId) ==
                ...
                case "Weapon.ProjectileHitReport":
                    if (!client.allowedProjectiles) {
                        server.sendChatTextToAdmins(`FairPlay: ${client.character.name} is hitting projectiles without ammunition`);
                        return;
                    }
                    client.allowedProjectiles--;
                    if (client.character.getEquippedWeapon().itemDefinitionId ==
                        enums_1.Items.WEAPON_REMOVER) {
                        if (!client.isAdmin)
                            return;
                        const characterId = p.packet.hitReport.characterId, entityType = server.getEntityType(characterId);
                        switch (entityType) {
                            case enums_1.EntityTypes.NPC:
                                if (!server._npcs[characterId]) {
                                    return;
                                }
                                server.deleteEntity(characterId, server._npcs);
                                break;
                            case enums_1.EntityTypes.VEHICLE:
                                const vehicle = server._vehicles[characterId];
                                if (!vehicle)
                                    return;
                                vehicle.destroy(server, true);
                                break;
                            case enums_1.EntityTypes.OBJECT:
                                const object = server._spawnedItems[characterId];
                                if (!object)
                                    return;
                                object.destroy(server);
                                break;
                            case enums_1.EntityTypes.EXPLOSIVE:
                                server.deleteEntity(characterId, server._explosives);
                                break;
                            case enums_1.EntityTypes.CONSTRUCTION_DOOR:
                            case enums_1.EntityTypes.CONSTRUCTION_SIMPLE:
                            case enums_1.EntityTypes.CONSTRUCTION_FOUNDATION:
                            case enums_1.EntityTypes.LOOTABLE_CONSTRUCTION:
                ...
                case "Weapon.ReloadRequest":
                    ...
                    server.sendRemoteWeaponUpdateDataToAllOthers(client, client.character.transientId, weaponItem.itemGuid, "Update.Reload", {});
                    const weaponAmmoId = server.getWeaponAmmoId(weaponItem.itemDefinitionId), maxAmmo = server.getWeaponMaxAmmo(weaponItem.itemDefinitionId), // max clip size
                    reloadTime = server.getWeaponReloadTime(weaponItem.itemDefinitionId);
                    if (weaponItem.itemDefinitionId == enums_1.Items.WEAPON_BOW_MAKESHIFT ||
                        weaponItem.itemDefinitionId == enums_1.Items.WEAPON_BOW_RECURVE ||
                        weaponItem.itemDefinitionId == enums_1.Items.WEAPON_BOW_WOOD) {
                        if (client.character.getEquippedWeapon().itemGuid !=
                            weaponItem.itemGuid)
                            return;
                        ...
                        reserveAmmo = // how much ammo is in inventory
                         client.character.getInventoryItemAmount(weaponAmmoId), reloadAmount = reserveAmmo >= maxReloadAmount ? maxReloadAmount : reserveAmmo; // actual amount able to reload
                    ...
                    //#region SHOTGUN ONLY
                    if (weaponAmmoId == enums_1.Items.AMMO_12GA) {
                        weaponItem.weapon.reloadTimer = setTimeout(() => {
                            ...
                            if (!reserveAmmo ||
                                ...
                                server.sendRemoteWeaponUpdateDataToAllOthers(client, client.character.transientId, weaponItem.itemGuid, "Update.ReloadLoopEnd", {
                                    endLoop: true,
                                });
                                client.character.clearReloadTimeout();
                                return;
                ...
                case "Weapon.SwitchFireModeRequest":
                    ...
                    if (p.packet.firemodeIndex == 1 &&
                        ...
                        return;
                    server.sendRemoteWeaponUpdateDataToAllOthers(client, client.character.transientId, weaponItem.itemGuid, "Update.SwitchFireMode", {
                ...
                case "Weapon.AimBlockedNotify":
                    server.sendRemoteWeaponUpdateDataToAllOthers(client, client.character.transientId, weaponItem.itemGuid, "Update.AimBlocked", {
    ...
    CommandRun(server, client, packet) {
        this.commandHandler.executeInternalCommand(server, client, "run", packet);
    }
    CommandSpectate(server, client, packet) {
        this.commandHandler.executeInternalCommand(server, client, "spectate", packet);
    }
    VoiceRadioChannel(server, client, packet) {
        if (!client.character._loadout[enums_1.LoadoutSlots.RADIO])
            return;
        if (client.character._loadout[enums_1.LoadoutSlots.RADIO].itemDefinitionId !=
            enums_1.Items.EMERGENCY_RADIO)
            return;
        client.radio = true;
    }
    VoiceLeaveRadio(server, client, packet) {
        client.radio = false;
    }
    EndCharacterAccess(server, client, packet) {
        client.character.dismountContainer(server);
    }
    //#endregion
    processPacket(server, client, packet) {
        switch (packet.name) {
            case "ClientIsReady":
...
exports.zonePacketHandlers = zonePacketHandlers;
//# sourceMappingURL=zonepackethandlers.js.map