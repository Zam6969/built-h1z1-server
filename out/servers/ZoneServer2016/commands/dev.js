...
const dev = {
    path: function (server, client, args) {
        const characterId = server.generateGuid();
        const npc = new baselightweightcharacter_1.BaseLightweightCharacter(characterId, server.getTransientId(characterId), 9510, client.character.state.position, client.character.state.rotation, server);
    ...
    zombie: function (server, client, args) {
        // spawn a zombie
        const characterId = server.generateGuid();
        const transient = server.getTransientId(characterId);
        const zombie = new npc_1.Npc(characterId, transient, 9510, client.character.state.position, client.character.state.rotation, server);
        server._npcs[characterId] = zombie;
    },
    zombiemove: function (server, client, args) {
        // spawn a zombie
        const characterId = server.generateGuid();
        const transient = server.getTransientId(characterId);
        const zombie = new npc_1.Npc(characterId, transient, 9510, client.character.state.position, client.character.state.rotation, server);
    ...
    stats: function (server, client, args) {
        server.logStats();
    },
    spam: function (server, client, args) {
    ...
    createcustomloadout: function (server, client, args) {
        if (!args[2]) {
    ...
    setslot: function (server, client, args) {
        if (!args[2]) {
    ...
    stat: function (server, client, args) {
        if (!args[3]) {
    ...
    listcontainers: function (server, client, args) {
    ...
    begincharacteraccess: function (server, client, args) {
        const objectCharacterId = server.generateGuid(), npc = new npc_1.Npc(objectCharacterId, server.getTransientId(objectCharacterId), 9034, client.character.state.position, client.character.state.lookAt, server);
    ...
    fte: function (server, client, args) {
        if (!args[3]) {
    ...
    proximateitems: function (server, client, args) {
        const item = server.generateItem(2425)?.itemGuid, guid1 = server.generateGuid(), guid2 = server.generateGuid(), guid3 = server.generateGuid();
    ...
    //endregion
    */
    poi: function (server, client, args) {
        server.sendData(client, "POIChangeMessage", {
            messageStringId: Number(args[1]) || 0,
            id: Number(args[1]) || 0,
        });
    },
    vehicleaccess: function (server, client, args) {
    ...
    stop: function (server, client, args) {
