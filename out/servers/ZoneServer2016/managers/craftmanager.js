...
exports.CraftManager = void 0;
const enums_1 = require("../models/enums");
const utils_1 = require("../../../utils/utils");
const debug = require("debug")("ZoneServer");
function getCraftComponentsDataSource(client) {
...
class CraftManager {
    constructor(client, server, recipeId, count) {
        this.craftLoopCount = 0;
        this.maxCraftLoopCount = 500;
        this.componentsDataSource = {};
    ...
    removeCraftComponent(itemDefinitionId, count) {
    ...
    async craftItem(server, client, recipeId, recipeCount) {
        ...
        const recipe = server._recipes[recipeId], bundleCount = recipe?.bundleCount || 1, // the amount of an item crafted from 1 recipe (ex. crafting 1 stick recipe gives you 2)
        craftCount = recipeCount * bundleCount; // the actual amount of items to craft
        if (!recipe)
            return false;
        switch (recipe.filterId) {
            case enums_1.FilterIds.COOKING:
            case enums_1.FilterIds.FURNACE:
                server.sendAlert(client, "This recipe requires a furnace, barbeque, or campfire to craft");
                return false;
        }
        if (recipe.requireWorkbench) {
            if (!(0, utils_1.checkConstructionInRange)(server._constructionSimple, client.character.state.position, 1.5, enums_1.Items.WORKBENCH) &&
                !(0, utils_1.checkConstructionInRange)(server._worldSimpleConstruction, client.character.state.position, 1.5, enums_1.Items.WORKBENCH)) {
        ...
        for (const component of recipe.components) {
            ...
            if (!this.componentsDataSource[component.itemDefinitionId]) {
                const componentRecipe = server._recipes[component.itemDefinitionId], componentBundleCount = componentRecipe?.bundleCount || 1;
                if (!componentRecipe) {
                ...
                if (component.requiredAmount / componentBundleCount < 1) {
                    if (!(await this.craftItem(server, client, component.itemDefinitionId, Math.ceil((component.requiredAmount / componentBundleCount) * recipeCount)))) {
                        return false; // craftItem returned some error
                    }
                }
                else {
                    for (let i = 0; i < recipeCount; i++) {
                        if (!(await this.craftItem(server, client, component.itemDefinitionId, Math.ceil(component.requiredAmount / componentBundleCount)))) {
                            return false; // craftItem returned some error
                        }
                    }
                }
            ...
            else if (this.componentsDataSource[component.itemDefinitionId].stackCount <
                remainingItems) {
                const componentRecipe = server._recipes[component.itemDefinitionId], componentBundleCount = componentRecipe?.bundleCount || 1;
                if (!componentRecipe) {
                    debug(`[CraftManager] ${client.character.name} tried to craft an invalid recipe ${component.itemDefinitionId}!`);
                ...
                if (component.requiredAmount / componentBundleCount < 1) {
                    ...
                    if (!(await this.craftItem(server, client, component.itemDefinitionId, craft))) {
                        return false; // craftItem returned some error
                    }
                }
                else {
                    for (let i = 0; i < recipeCount; i++) {
                        ...
                        if (!(await this.craftItem(server, client, component.itemDefinitionId, Math.ceil(craftAmount / componentBundleCount)))) {
                            return false; // craftItem returned some error
                        }
                    }
                }
        ...
        //#endregion
        //#region CRAFTING
        await server.pUtilizeHudTimer(client, server.getItemDefinition(recipeId).NAME_ID, 1000 * recipeCount);
        ...
        for (const component of r.components) {
            ...
            let remainingItems = component.requiredAmount * recipeCount, stackCount = 0;
        ...
        client.character.lootItem(server, server.generateItem(recipeId, craftCount));
        return true;
        //#endregion
    }
    async start(client, server, recipeId, count) {
        for (let i = 0; i < count; i++) {
            if (!(await this.craftItem(server, client, recipeId, 1)))
                return;
        }
    }
}
exports.CraftManager = CraftManager;
//# sourceMappingURL=craftmanager.js.map