...
const hax = {
    list: function (server, client, args) {
        server.sendChatText(client, `/hax commands list: \n/hax ${Object.keys(this).join("\n/hax ")}`);
    },
    placement: function (server, client, args) {
    ...
    siren: function (server, client, args) {
    ...
    spectate: function (server, client, args) {
        const characterId = server.generateGuid();
        const vehicleData = new vehicle_1.Vehicle(server._worldId, characterId, server.getTransientId(characterId), 9371, client.character.state.position, client.character.state.lookAt);
        ...
        server._vehicles[characterId] = {
            ...
            onReadyCallback: (clientTriggered) => {
    ...
    headlights: function (server, client, args) {
        let headlightType;
        switch (client.vehicle.mountedVehicleType) {
            case "offroader":
                headlightType = 273;
    ...
    vehicleaction: function (server, client, args) {
        if (client.vehicle.mountedVehicle) {
            ...
            switch (type) {
                case "refuel":
                    server._vehicles[client.vehicle.mountedVehicle].npcData.resources.fuel = 10000;
                    server.updateResource(client, vehicle.npcData.characterId, vehicle.npcData.resources.fuel, 396, 50);
                    server.sendChatText(client, "Vehicle refueled");
                    break;
                case "repair":
                    server._vehicles[client.vehicle.mountedVehicle].npcData.resources.health = 100000;
                    server._vehicles[client.vehicle.mountedVehicle].npcData.destroyedState = 0;
                    server.updateResource(client, vehicle.npcData.characterId, vehicle.npcData.resources.health, 561, 1);
    ...
    drive: function (server, client, args) {
        ...
        const characterId = server.generateGuid();
        const vehicleData = new vehicle_1.Vehicle(server._worldId, characterId, server.getTransientId(characterId), driveModel, client.character.state.position, client.character.state.lookAt);
        server.sendDataToAll("PlayerUpdate.AddLightweightVehicle", vehicleData);
        vehicleData.isManaged = true;
        server._vehicles[characterId] = {
            ...vehicleData,
    ...
    weaponstance: function (server, client, args) {
        const stance = args[1];
        let weaponStance;
        switch (stance) {
            case "0":
    ...
    state: function (server, client, args) {
        const state = args[1];
        switch (state) {
            case "list":
                server.sendChatText(client, "Availables states : hidden, sit, autorun, cuffed, handsup");
                break;
            case "hidden":
        ...
        server.updateCharacterState(client, client.character.characterId, client.character.characterStates, true);
    },
    spawnvehicle: function (server, client, args) {
        if (!args[1]) {
            server.sendChatText(client, "[ERROR] Usage /hax spawnVehicle offroader/pickup/policecar");
            return;
        }
        let driveModel;
        switch (args[1]) {
    ...
    bandage: function (server, client, args) {
        ...
        if (character.resources.health < 10000) {
            ...
            server.updateResource(client, character.characterId, character.resources.health, 48, 1);
        }
    },
    ...
    globalheartattack: function (server, client, args) {
    ...
    spamoffroader: function (server, client, args) {
        for (let index = 0; index < 150; index++) {
            const vehicleData = {
                ...
                positionUpdate: server.createPositionUpdate(client.character.state.position, [0, 0, 0, 0]),
            };
    ...
    changestat: function (server, client, args) {
    ...
    removedynamicweather: async function (server, client, args) {
    ...
    weather: function (server, client, args) {
        ...
        if (!choosenTemplate) {
            server.sendChatText(client, "Please define a weather template to use (data/sampleData/weather.json)");
        }
        else {
    ...
    weapon: function (server, client, args) {
        ...
        if (!choosenWeapon) {
            server.sendChatText(client, "Please define the name of the weapon you wanna use ! see /hax weapon list'");
        }
        else {
            switch (choosenWeapon.toLowerCase()) {
                case "list":
                    server.sendChatText(client, "Availables weapons : ar, hatchet, torch, empty");
                    break;
                case "ar":
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 7)].modelName = "Weapon_M16A4_3p.adr";
                    ...
                    break;
                case "hatchet":
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 7)].modelName = "Weapon_Hatchet01_3p.adr";
                    server.sendDataToAll("Equipment.SetCharacterEquipment", {
                        profileId: 3,
                        characterId: client.character.characterId,
                        equipmentSlots: client.character.equipment.map((equipment) => {
                    ...
                    break;
                case "empty":
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 7)].modelName = "Weapon_Empty.adr";
                    server.sendDataToAll("Equipment.SetCharacterEquipment", {
                        profileId: 3,
                        characterId: client.character.characterId,
                        equipmentSlots: client.character.equipment.map((equipment) => {
                    ...
                    break;
                case "torch":
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 7)].modelName = "Weapon_Torch_3p.adr";
                    server.sendDataToAll("Equipment.SetCharacterEquipment", {
                        profileId: 3,
                        characterId: client.character.characterId,
                        equipmentSlots: client.character.equipment.map((equipment) => {
    ...
    outfit: function (server, client, args) {
        const choosenOutfit = args[1];
        if (!choosenOutfit) {
            server.sendChatText(client, "Please define the name of the outfit you wanna use ! see /hax outfit list'");
        }
        else {
            switch (choosenOutfit.toLowerCase()) {
                case "list":
                    server.sendChatText(client, "Availables outfits : Aviator, Cowboy, Jinx, Red");
                    break;
                case "aviator":
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 1)].modelName = "SurvivorMale_Ivan_AviatorHat_Base.adr";
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 3)].modelName = "SurvivorMale_Ivan_Shirt_Base.adr";
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 3)].defaultTextureAlias = "Ivan_Tshirt_Army_Green";
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 4)].modelName = "SurvivorMale_Ivan_Pants_Base.adr";
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 4)].defaultTextureAlias = "Ivan_Pants_Jeans_Black";
                    server.sendDataToAll("Equipment.SetCharacterEquipment", {
                        profileId: 3,
                        characterId: client.character.characterId,
                        equipmentSlots: client.character.equipment.map((equipment) => {
                    ...
                    break;
                case "jinx":
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 1)].modelName = "Weapon_Empty.adr";
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 3)].modelName = "SurvivorMale_Ivan_Shirt_Base.adr";
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 3)].defaultTextureAlias = "Ivan_Tshirt_JINX";
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 4)].modelName = "SurvivorMale_Ivan_Pants_Base.adr";
                    server.sendDataToAll("Equipment.SetCharacterEquipment", {
                        profileId: 3,
                        characterId: client.character.characterId,
                    ...
                    break;
                case "cowboy":
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 1)].modelName = "SurvivorMale_Ivan_OutbackHat_Base.adr";
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 1)].defaultTextureAlias = "Ivan_OutbackHat_LeatherTan";
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 3)].modelName = "SurvivorMale_Ivan_Shirt_Base.adr";
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 3)].defaultTextureAlias = "Ivan_Tshirt_Navy_Shoulder_Stripes";
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 4)].modelName = "SurvivorMale_Ivan_Pants_Base.adr";
                    server.sendDataToAll("Equipment.SetCharacterEquipment", {
                        profileId: 3,
                        characterId: client.character.characterId,
                    ...
                    break;
                case "red":
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 1)].modelName = "SurvivorMale_Ivan_Motorcycle_Helmet_Grey.adr";
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 1)].defaultTextureAlias = "Ivan_MotorCycle_Helmet_Red";
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 3)].modelName = "SurvivorMale_Ivan_Shirt_Base.adr";
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 3)].defaultTextureAlias = "Ivan_Tshirt_RAZOR";
                    client.character.equipment[client.character.equipment.findIndex((x) => x.slotId === 4)].modelName = "SurvivorMale_Ivan_Pants_Base.adr";
                    server.sendDataToAll("Equipment.SetCharacterEquipment", {
                        profileId: 3,
                        characterId: client.character.characterId,
    ...
    randomweather: function (server, client, args) {
        ...
        const rnd_weather = {
            ...
            unknownArray: utils_1._.fill(Array(50), {
                unknownDword1: 0,
                unknownDword2: 0,
                unknownDword3: 0,
                unknownDword4: 0,
    ...
    rick: function (server, client, args) {
    ...
    godmode: function (server, client, args) {
