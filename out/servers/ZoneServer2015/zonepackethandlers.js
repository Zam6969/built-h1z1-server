...
class zonePacketHandlers {
    constructor() {
        this.hax = hax;
        this.dev = dev;
        this.admin = admin_1.default;
        this.ClientIsReady = function (server, client, packet) {
            ...
            client, client.character.characterId, client.character.resources.health, 48, 1);
            const commands = [
                "hax",
                "dev",
                "admin",
            ...
            server.updateCharacterState(client, client.character.characterId, client.character.characterStates, false);
        ...
        this.ClientFinishedLoading = (server, client, packet) => {
            ...
            if (client.firstLoading) {
                ...
                client.savePositionTimer = setTimeout(() => server.saveCharacterPosition(client, true), 30000);
                if (!server._soloMode) {
        ...
        this.Security = function (server, client, packet) {
            debug(packet);
        };
        this.commandRecipeStart = function (server, client, packet) {
            debug(packet);
            server.sendData(client, "Command.RecipeAction", {});
        };
        this.commandFreeInteractionNpc = function (server, client, packet) {
            debug("FreeInteractionNpc");
            server.sendData(client, "Command.FreeInteractionNpc", {});
        };
        this.collisionDamage = function (server, client, packet) {
        ...
        this.lobbyGameDefinitionDefinitionsRequest = function (server, client, packet) {
            server.sendData(client, "LobbyGameDefinition.DefinitionsResponse", {
                definitionsData: { data: "" },
            });
        };
        this.VehicleItemDefinitionRequest = function (server, client, packet) {
            debug(`Character "${client.character.name}" (${client.character.characterId}) ask for VehicleItemDefinition`);
        };
        this.CurrentMoveMode = function (server, client, packet) {
            debug(`Vehicle "${packet.data.characterId}" move mode : ${packet.data.moveMode}`);
        };
        this.playerUpdateEndCharacterAccess = function (server, client, packet) {
            debug("EndCharacterAccess");
        };
        this.KeepAlive = function (server, client, packet) {
            server.sendData(client, "KeepAlive", {
                gameTime: packet.data.gameTime,
            });
        };
        this.ClientMetrics = function (server, client, packet) {
            debugWOD(packet);
        };
        this.WallOfDataClientSystemInfo = function (server, client, packet) {
            debugWOD(packet.data.ClientSystemInfo);
        };
        this.WallOfDataLaunchPadFingerprint = function (server, client, packet) {
            debugWOD(`LaunchPadFingerprint : ${packet.data.LaunchPadFingerprint}`);
        };
        this.ClientLog = function (server, client, packet) {
            debug(packet);
        };
        this.wallOfDataUIEvent = function (server, client, packet) {
            debug(packet);
        };
        this.SetLocale = function (server, client, packet) {
            debug("Do nothing");
        };
        this.GetContinentBattleInfo = function (server, client, packet) {
        ...
        this.chatChat = function (server, client, packet) {
            const { channel, message } = packet.data;
            server.sendChat(client, message, channel);
        };
        this.loadoutSelectSlot = function (server, client, packet) {
            /*
        ...
        this.ClientInitializationDetails = function (server, client, packet) {
            // just in case
            if (packet.data.unknownDword1) {
                debug("ClientInitializationDetails : ", packet.data.unknownDword1);
            }
        };
        this.ClientLogout = function (server, client, packet) {
            debug("ClientLogout");
            clearTimeout(client.hudTimer); // clear the timer started at StartLogoutRequest
            server.deleteClient(client);
        };
        this.GameTimeSync = function (server, client, packet) {
            server.sendGameTimeSync(client);
        };
        this.Synchronization = function (server, client, packet) {
            const serverTime = (0, utils_1.Int64String)(server.getSequenceTime());
        ...
        this.commandExecuteCommand = async function (server, client, packet) {
            const args = packet.data.arguments.toLowerCase().split(" ");
            const commandName = args[0];
            switch (packet.data.commandHash) {
                case 3720768430: // /respawn
                    server.killCharacter(client);
                    break;
                case (0, h1emu_core_1.joaat)("NETSTATS"):
                ...
                case 2371122039: // /serverinfo
                    if (commandName === "mem") {
                        ...
                        const { _clients: clients, _characters: characters, _npcs: npcs, _objects: objects, _vehicles: vehicles, _doors: doors, _props: props, } = server;
                        const delta = Date.now() - server._startTime;
                        const datakur = new Date((server._serverTime + delta) * server._timeMultiplier);
                        const monthNames = [
                            "January",
                            "February",
                            "March",
                        ...
                        server.sendChatText(client, "Gametime: " +
                            datakur.getUTCDate() +
                            " " +
                ...
                case 1757604914: // /spawninfo
                    ...
                    break;
                case (0, h1emu_core_1.joaat)("HELP"):
                case 3575372649: // /help
                    ...
                    break;
                case (0, h1emu_core_1.joaat)("LOCATION"):
                case 3270589520: // /loc
                ...
                case (0, h1emu_core_1.joaat)("HAX"):
                    if (client.isAdmin ||
                        ...
                        if (!server._allowedCommands.includes(commandName)) {
                            ...
                            server.sendChatText(client, `Unknown command: /hax ${commandName} , display hax all commands by using /hax list`);
                        }
                    }
                    break;
                case (0, h1emu_core_1.joaat)("DEV"):
                case 552078457: // dev
                    if (client.isAdmin ||
                        commandName === "list" ||
                        ((server._allowedCommands.length === 0 ||
                    ...
                    break;
                case (0, h1emu_core_1.joaat)("ADMIN"):
                case 997464845: // admin
                    if (client.isAdmin ||
                        ...
                        if (!server._allowedCommands.includes(commandName)) {
                            ...
                            server.sendChatText(client, `Unknown command: /admin ${commandName} , display admin all commands by using /admin list`);
                        }
                    }
                    break;
            }
        };
        this.commandSetProfile = function (server, client, packet) {
            server.sendData(client, "Loadout.SetCurrentLoadout", {
                type: 2,
                unknown1: 0,
                loadoutId: 15,
        ...
        this.playerUpdateWeaponStance = function (server, client, packet) {
            server.sendDataToAll("PlayerUpdate.WeaponStance", {
                characterId: client.character.characterId,
                stance: packet.data.stance,
            });
        };
        this.mountSeatChangeRequest = function (server, client, packet) {
        ...
        this.mountDismountRequest = function (server, client, packet) {
            if (client?.vehicle?.mountedVehicle)
                // TODO: fix that in a better way
                server.dismountVehicle(client, client.vehicle.mountedVehicle);
        };
        this.commandInteractRequest = function (server, client, packet) {
        ...
        this.commandInteractionString = function (server, client, packet) {
            const { guid } = packet.data, entityData = server._objects[guid] ||
                server._vehicles[guid] ||
                server._doors[guid] ||
                server._props[guid], entityType = server._objects[guid]
                ? 1
                : 0 || server._vehicles[guid]
                    ...
                    : 0 || server._doors[guid]
                        ...
                        : 0 || server._props[guid]
                            ...
                            : 0;
            if (!entityData ||
                !(0, utils_1.isPosInRadius)(server._interactionDistance, client.character.state.position, entityData.npcData ? entityData.npcData.position : entityData.position))
                return;
            switch (entityType) {
                case 1: // object
        ...
        this.commandSetInWater = function (server, client, packet) {
            // nothing
        };
        this.commandClearInWater = function (server, client, packet) {
            // nothing
        };
        this.commandInteractionSelect = function (server, client, packet) {
            debug(packet);
            debug("select");
        };
        this.playerUpdateVehicleCollision = function (server, client, packet) {
            debug(packet);
        };
        this.vehicleDismiss = function (server, client, packet) {
        ...
        this.vehicleSpawn = function (server, client, packet) {
        ...
        this.vehicleAutoMount = function (server, client, packet) {
        ...
        this.commandRedeploy = function (server, client, packet) {
            debug("Redeploy");
        ...
        this.commandInteractCancel = function (server, client, packet) {
            debug("Interaction Canceled");
        };
        this.commandStartLogoutRequest = function (server, client, packet) {
        ...
        this.CharacterSelectSessionRequest = function (server, client, packet) {
            server.sendData(client, "CharacterSelectSessionResponse", {
                status: 1,
                sessionId: client.loginSessionId,
            });
        };
        this.profileStatsGetPlayerProfileStats = function (server, client, packet) {
            server.sendData(client, "ProfileStats.PlayerProfileStats", require("../../../data/2015/sampleData/profilestats.json"));
        };
        this.DtoHitSpeedTreeReport = function (server, client, packet) {
            debug(packet);
            server.speedTreeUse(client, packet);
        };
        this.GetRewardBuffInfo = function (server, client, packet) {
        ...
        this.vehicleStateData = function (server, client, packet) {
        ...
        this.VehicleAccessType = function (server, client, packet) {
        ...
        this.PlayerUpdateManagedPosition = function (server, client, packet) {
            if (!packet.data) {
                return;
            }
                ...
                if (packet.data.PositionUpdate.position) {
                    ...
                    if (client.vehicle.mountedVehicle === characterId) {
                        ...
                        if (server._vehicles[characterId].passengers.passenger1) {
                            const character = server._vehicles[characterId].passengers.passenger1;
                            server.updatePosition(character, packet.data.PositionUpdate.position);
                        }
                        if (server._vehicles[characterId].passengers.passenger2) {
                            const character = server._vehicles[characterId].passengers.passenger2;
                            server.updatePosition(character, packet.data.PositionUpdate.position);
                        }
                        if (server._vehicles[characterId].passengers.passenger3) {
                            const character = server._vehicles[characterId].passengers.passenger3;
                            server.updatePosition(character, packet.data.PositionUpdate.position);
                        }
                        if (server._vehicles[characterId].passengers.passenger4) {
                            const character = server._vehicles[characterId].passengers.passenger4;
                            server.updatePosition(character, packet.data.PositionUpdate.position);
                        }
                        if (!client.posAtLastRoutine ||
                            !(0, utils_1.isPosInRadius)(server._npcRenderDistance *
                                server._worldRoutineRadiusPercentage, client.character.state.position, client.posAtLastRoutine)) {
                            server.worldRoutine();
                        }
                    }
                }
        ...
        this.PlayerUpdateUpdatePositionClientToZone = function (server, client, packet) {
            ...
            if (packet.data.position) {
                ...
                if (!client.posAtLastRoutine ||
                    (!(0, utils_1.isPosInRadius)(server._npcRenderDistance / 2.5, client.character.state.position, client.posAtLastRoutine) &&
                        !client.isLoading)) {
                    server.worldRoutine();
                }
            }
        ...
        this.commandPlayerSelect = function (server, client, packet) {
            debug(packet);
            const { guid } = packet.data, entityData = server._objects[guid] ||
                server._vehicles[guid] ||
                server._doors[guid] ||
            ...
            switch (entityType) {
                case 1: // object
                    ...
                    const model_index = modelToName.findIndex((x) => x.modelId === entityData.modelId);
                    ...
                    switch (entityData.modelId) {
                        case 9159:
                            ...
                            server.updateResource(client, client.character.characterId, client.character.resources.water, 5, 5);
                            break;
                        case 8020:
                        case 9250:
                            client.character.resources.food = food + 4000;
                            server.updateResource(client, client.character.characterId, client.character.resources.food, 4, 4);
                            break;
                        case 9221:
                            client.character.resources.health = health + 10000;
                            server.updateResource(client, client.character.characterId, client.character.resources.health, 48, 1);
                            break;
                        default:
                            break;
                    }
        ...
        this.constructionPlacementRequest = function (server, client, packet) {
            debug("Construction.PlacementRequest");
            // TODO
            //server.sendData(client, "Construction.PlacementResponse", {model:modelChoosen});
        };
        this.constructionPlacementFinalizeRequest = function (server, client, packet) {
            debug(packet);
        ...
        this.playerUpdateRespawn = function (server, client, packet) {
            debug(packet);
            server.respawnPlayer(client);
        };
        this.playerUpdateFullCharacterDataRequest = function (server, client, packet) {
            const { characterId } = packet.data;
            const entityType = server.getEntityType(characterId);
            switch (entityType) {
                case 1: {
                    // npc
                ...
                case 3: {
                    ...
                    if (entityData) {
                        ...
                        server.sendData(client, "Equipment.SetCharacterEquipment", {
                            profileId: 3,
                            characterId: entityData.characterId,
                            equipmentSlots: entityData.equipment.map((equipment) => {
                                return {
