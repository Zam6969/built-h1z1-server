...
class GatewayServer extends events_1.EventEmitter {
    constructor(serverPort, gatewayKey) {
        ...
        this._crcLength = 0;
        this._udpLength = 512;
        this._soeServer = new soeserver_1.SOEServer(serverPort, gatewayKey);
        ...
        this._soeServer.on("appdata", (client, data, isRawData) => {
            if (isRawData) {
                ...
                switch (packet.name) {
                    case "LoginRequest":
                        if (packet.character_id) {
                            ...
                            this.emit("login", client, packet.character_id, packet.ticket, packet.client_protocol);
                        }
                        break;
                    case "Logout":
    ...
    _sentTunnelData(client, tunnelData, unbuffered) {
    ...
    sendTunnelData(client, tunnelData) {
        this._sentTunnelData(client, tunnelData, false);
    }
    sendUnbufferedTunnelData(client, tunnelData) {
