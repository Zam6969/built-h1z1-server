...
const shared_1 = require("../packets/ClientProtocol/ClientProtocol_1080/shared");
...
class H1Z1Protocol {
    ...
    createPositionBroadcast(rawData, transientId) {
        const tId = (0, shared_1.packUnsignedIntWith2bitLengthValue)(transientId);
        return Buffer.concat([new Uint8Array([120]), tId, rawData]);
    }
    createPositionBroadcast2016(rawData, transientId) {
        const tId = (0, shared_1.packUnsignedIntWith2bitLengthValue)(transientId);
        return Buffer.concat([Buffer.from([0x79]), tId, rawData]); //0x79 = opcode
    }
    createVehiclePositionBroadcast(rawData) {
        return Buffer.concat([new Uint8Array([120]), rawData]);
    }
    createVehiclePositionBroadcast2016(rawData, transientId) {
        const tId = (0, shared_1.packUnsignedIntWith2bitLengthValue)(transientId);
        return Buffer.concat([Buffer.from([0x91]), tId, rawData]); //0x91 = opcode
    }
    parseFacilityReferenceData(data) {
        ...
        const result = h1z1_dataschema_1.default.parse(schema, data, 0).result;
        return result;
    }
    parseWeaponDefinitionReferenceData(data) {
        const inSize = data.readUInt32LE(0), outSize = data.readUInt32LE(4), compData = data.slice(8);
        data = (0, utils_1.lz4_decompress)(compData, inSize, outSize);
    ...
    parseUpdatePositionClientToZone(data, offset) {
        return {
            result: parseUpdatePositionData(data, offset),
        };
    }
    parseUpdatePositionRaw(data) {
        // Temp workaround
        const obj = {};
        obj.raw = data;
        return {
    ...
    parseUpdatePositionZoneToClient(data, offset) {
        const obj = {};
    ...
    pack(packetName, object = {}) {
        ...
        const packetType = H1Z1Packets.PacketTypes[packetName];
        ...
        if (packet) {
            ...
            if (packet.schema) {
                try {
                    packetData = h1z1_dataschema_1.default.pack(packet.schema, object, null, null);
                }
                catch (error) {
    ...
    parse(data, flag) {
        ...
        if (packet) {
            if (packet.schema) {
                ...
                try {
                    result = h1z1_dataschema_1.default.parse(packet.schema, data, offset).result;
                }
                catch (e) {
    ...
    reloadPacketDefinitions() {
        ...
        (0, utils_1.clearFolderCache)(__dirname, protocolPacketsFolderPath);
        this.H1Z1Packets = require(`${protocolPacketsFolderPath}/h1z1packets.js`);
    }
}
exports.H1Z1Protocol = H1Z1Protocol;
const readSignedIntWith2bitLengthValue = function (data, offset) {
...
const readUnsignedIntWith2bitLengthValue = function (data, offset) {
    let value = data.readUInt8(offset);
    const n = value & 3;
    for (let i = 0; i < n; i++) {
        value += data.readUInt8(offset + i + 1) << ((i + 1) * 8);
...
const parseUpdatePositionData = function (data, offset) {
    const obj = {};
    obj.raw = data;
    try {
...
//# sourceMappingURL=h1z1protocol.js.map