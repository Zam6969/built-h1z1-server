...
exports.packRemoteWeaponUpdatePacket = exports.packRemoteWeaponPacket = exports.packWeaponPacket = exports.parseWeaponPacket = void 0;
                        ...
                        { name: "unknownBoolean1", type: "boolean", defaultValue: false },
                        {
                            ...
                            fields: [
                                { name: "statCategory", type: "uint8", defaultValue: 0 },
                                {
                                    name: "statUpdateData",
                                    type: "schema",
                                    fields: shared_2.itemWeaponDetailSubSchema1,
                                },
                            ],
                        },
                    ],
            ...
            fields: [
                {
                    ...
                    fields: [
                        ...
                        },
                        {
                            ...
                            fields: [
                                { name: "guid", type: "uint64string", defaultValue: "" },
                                ...shared_1.remoteWeaponExtraSchema,
                            ],
                        },
                    ],
                },
        ...
        0x02,
        {
            fields: [
                { name: "guid", type: "uint64string", defaultValue: "" },
                {
                    name: "data",
                    type: "byteswithlength",
                    fields: shared_1.remoteWeaponSchema,
                },
            ],
        },
    ],
...
const remoteWeaponUpdatePackets = [
    [
        "Update.FireState",
        0x01,
        {
...
const [weaponPacketTypes, weaponPacketDescriptors] = (0, packettable_1.default)(weaponPackets), [remoteWeaponPacketTypes, remoteWeaponPacketDescriptors] = (0, packettable_1.default)(remoteWeaponPackets), [remoteWeaponUpdatePacketTypes, remoteWeaponUpdatePacketDescriptors] = (0, packettable_1.default)(remoteWeaponUpdatePackets);
...
function parseWeaponPacket(data, offset) {
    ...
    const weaponPacket = (0, shared_1.readPacketType)(weaponPacketData, weaponPacketDescriptors), packetType = `0x${weaponPacket.packetType.toString(16).slice(2)}`;
    if (weaponPacket.packet) {
        obj.packetType = packetType;
        obj.packetName = weaponPacket.packet.name;
        if (weaponPacket.packet.schema) {
            obj.packet = h1z1_dataschema_1.default.parse(weaponPacket.packet.schema, weaponPacketData, weaponPacket.length).result;
        }
    }
    else {
        obj.packetType = packetType;
...
exports.parseWeaponPacket = parseWeaponPacket;
function packWeaponPacket(obj) {
    ...
    if (weaponPacketDescriptors[subType]) {
        const subPacket = weaponPacketDescriptors[subType], subTypeData = (0, shared_1.writePacketType)(subType);
...
exports.packWeaponPacket = packWeaponPacket;
function packFirestateUpdate(obj) {
    ...
    if ((obj.firestate & 2) == 0) {
        ...
        data = Buffer.concat([
            data,
            h1z1_dataschema_1.default.pack([
                {
                    name: "transientId",
                    type: "custom",
...
function packRemoteWeaponPacket(obj) {
    if (obj.remoteWeaponPacket.packetName == "RemoteWeapon.Update")
        return packRemoteWeaponUpdatePacket(obj);
    const subObj = obj.remoteWeaponPacket.packet, subName = obj.remoteWeaponPacket.packetName, subType = remoteWeaponPacketTypes[subName];
    if (!remoteWeaponPacketDescriptors[subType]) {
        throw "Unknown weapon packet type: " + subType;
    }
    let subData = Buffer.allocUnsafe(6);
    const subTypeData = (0, shared_1.writePacketType)(subType);
    ...
    const transientId = (0, shared_1.packUnsignedIntWith2bitLengthValue)(obj["remoteWeaponPacket"]["transientId"]);
    subData = Buffer.concat([subData, transientId]);
    const packetData = h1z1_dataschema_1.default.pack(remoteWeaponPacketDescriptors[subType].schema, subObj).data;
    return Buffer.concat([subData, packetData]);
}
exports.packRemoteWeaponPacket = packRemoteWeaponPacket;
function packRemoteWeaponUpdatePacket(obj) {
    const subObj = obj.remoteWeaponPacket.remoteWeaponUpdatePacket.packet, subName = obj.remoteWeaponPacket.remoteWeaponUpdatePacket.packetName, subType = remoteWeaponUpdatePacketTypes[subName];
    ...
    const packetData = h1z1_dataschema_1.default.pack(remoteWeaponUpdatePacketDescriptors[subType].schema, subObj).data;
    return Buffer.concat([subData, packetData]);
}
exports.packRemoteWeaponUpdatePacket = packRemoteWeaponUpdatePacket;
...
function parseHitReportPacket(data, offset) {
    const obj = h1z1_dataschema_1.default.parse(hitReportSchema, data, offset).result;
    offset += 26 + obj.hitLocationLen;
    let byteLen = 8;
    if (obj.hitLocationLen) {
        byteLen = 9;
    }
    obj.unknownBytes = h1z1_dataschema_1.default.parse([{ name: "unknownBytes", type: "bytes", length: byteLen }], data, 0).result;
    offset += byteLen;
