...
function packWeaponDefinitionData(obj) {
    const compressionData = Buffer.allocUnsafe(8), data = h1z1_dataschema_1.default.pack(weaponDefinitionSchema, obj).data, input = data;
...
const projectileDefinitionSchema = [
    {
...
function packProjectileDefinitionData(obj) {
    const compressionData = Buffer.allocUnsafe(8), data = h1z1_dataschema_1.default.pack(projectileDefinitionSchema, obj).data, input = data;
    let output = Buffer.alloc(utils_1.LZ4.encodeBound(input.length));
    output = output.slice(0, utils_1.LZ4.encodeBlock(input, output));
    compressionData.writeUInt32LE(output.length, 0);
    compressionData.writeUInt32LE(data.length, 4);
